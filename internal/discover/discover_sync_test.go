...
func TestDiscoverThenCaptureReAdd(t *testing.T) {
	...
	mock.OnCommandSuccess(
		testutil.MatchCommandPrefix("chezmoi", "--source", filepath.Join(repoDir, "home"), "add"),
		"",
	)
	mock.OnCommandSuccess(
		testutil.MatchCommandPrefix("chezmoi", "--source", filepath.Join(repoDir, "home"), "re-add"),
		"",
	)
	...
	addCall := findCall(mock.Calls(), testutil.MatchCommandPrefix("chezmoi", "--source", filepath.Join(repoDir, "home"), "add"))
	if addCall == nil {
	...
	addIndex := findCallIndex(mock.Calls(), testutil.MatchCommandPrefix("chezmoi", "--source", filepath.Join(repoDir, "home"), "add"))
	reAddIndex := findCallIndex(
		mock.Calls(),
		testutil.MatchCommandPrefix("chezmoi", "--source", filepath.Join(repoDir, "home"), "re-add"),
	)
...
func TestDiscoverUsesWarningSecretsModeOnAdd(t *testing.T) {
	ctx := testutil.ContextWithCancel(t)
	repoDir := testutil.TempDir(t)
	homeDir := testutil.TempDir(t)
	...
	mock := testutil.NewMockRunner(t)
	mock.OnCommandSuccess(
		testutil.MatchCommandPrefix("chezmoi", "--source", filepath.Join(repoDir, "home"), "add", "--secrets=warning"),
		"",
	)
	...
	if err := discoverer.Run(ctx, Options{AutoYes: true, NoCommit: true, SecretsMode: "warning"}); err != nil {
		t.Fatalf("discover run failed: %v", err)
	}

	addCall := findCall(mock.Calls(), testutil.MatchCommandPrefix("chezmoi", "--source", filepath.Join(repoDir, "home"), "add", "--secrets=warning"))
	if addCall == nil {
		t.Fatal("expected chezmoi add to be invoked with --secrets=warning")
	}
	assertArgsContain(t, addCall.Args, managedPath)
}

func TestDiscoverSkipsAutoCommitWhenRepoAlreadyDirty(t *testing.T) {
	ctx := testutil.ContextWithCancel(t)
	repoDir := testutil.TempDir(t)
	homeDir := testutil.TempDir(t)
	...
	if err := discoverer.Run(ctx, Options{AutoYes: true, NoCommit: false, SecretsMode: "ignore"}); err != nil {
		t.Fatalf("discover run failed: %v", err)
	}
	...
	assertArgsContain(t, addCall.Args, managedPath)
	mock.AssertNotCalled(testutil.MatchExact("git", "add", "-A"))
